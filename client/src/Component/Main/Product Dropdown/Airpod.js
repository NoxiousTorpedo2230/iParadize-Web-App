...
import { airpodFilter } from "../Product/ProductFilter";
import { addToCart, handleViewDetails, handleCloseModal, handleBuyNow } from "../Product/ProductHandlers";

function Airpod({ searchTerm, setCartItems, handleAddToCart }) {
  const [selectedProduct, setSelectedProduct] = useState(null);
  const [showModal, setShowModal] = useState(false);
  const userId = localStorage.getItem("userId");

  const filteredProducts = airpodFilter({ searchTerm, setCartItems });
            ...
            filteredProducts.map((product, index) => (
              <Col key={index} lg={3} md={6} sm={12} className="mb-4">
                <ProductCard
                  ...
                  onViewDetails={() => handleViewDetails(product, setSelectedProduct, setShowModal)}
                  onBuyNow={() => handleBuyNow(product, () => handleCloseModal(setShowModal, setSelectedProduct))}
                />
              </Col>
            ))
          ) : (
